<template>
	<div class="content-fluid">
		<portal to="header-title">
			<span>Company</span>
			<nuxt-link tag="a" to="/companies/create" class="btn btn-primary" style="margin-left:1%;">
				<span><i class="fas fa-building"></i> Add</span>
			</nuxt-link>
		</portal>
		<div class="row">
			<div class="col-12">
	...
	data() {
		return {
			...
			columns: [
				...
				{ 
					field: "name", key: "b", title: "Company", align: "left", sortBy: "", 
					renderBodyCell: ({row, column, rowIndex}, h)=> {
				...
				{ 
					field: "counter_employees", key: "g", title: "Employees", align: "center", sortBy: "",
					renderBodyCell: ({row, column, rowIndex}, h)=> {
						if (!row['counter_employees']) {
							return '-';
						}
						
						return (
							<button type="button" class="btn btn-app" 
								on-click:prevent={()=> this.$router.push({name: `employee.index`, query: {company: row['id']}})}
							>
								<i class="fas fa-user-tie"></i>
								<b>See All ({ row['counter_employees'] })</b>
							</button>
						);
					}, 
				},
				{ field: "phone_number", key: "c", title: "Phone", align: "left", sortBy: "" },
				{ field: "official_email_address", key: "d", title: "Email", align: "left", sortBy: "" },
				{
					field: "created_at",
					key: "e",
					title: "Created At",
					...
					renderBodyCell: ({row, column, rowIndex}, h)=> {
						return (
							<div class="btn-group btn-block">
								<button type="button" class="btn btn-warning btn-sm" on-click={()=> this.$router.push(`companies/${row['id']}/edit`)}>
	...
	methods: {
		...
		deleteRow(id){
			...
			this.$swal.fire({
				...
				if (result && result.dismiss) {
					this.$swal.fire('Company is not deleted', '', 'info');
				}
			})
