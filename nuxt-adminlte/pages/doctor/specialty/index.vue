<template>
	<div class="content-fluid">
		<portal to="header-title">
			...
			<nuxt-link tag="a" to="/doctors/specialties/create" class="btn btn-primary" style="margin-left:1%;">
				<span><i class="fas fa-user-tag"></i> Add</span>
			</nuxt-link>
		</portal>
		<div class="row">
			<div class="col-12">
...
export default {
	data() {
		return {
			...
			columns: [
				...
				{ 
					field: "specialty", key: "b", title: "Specialty", align: "left", sortBy: "",
					renderBodyCell: ({row, column, rowIndex}, h)=> {
				...
				// { 
				// 	key: "g", title: "Doctors", align: "left", sortBy: "",
				// 	renderBodyCell: ({row, column, rowIndex}, h)=> {
				...
				// 				on-click:prevent={()=> this.$router.push({name: `doctor.index`, query: {specialty: row['id']}})}
				// 			>
					...
					renderBodyCell: ({row, column, rowIndex}, h)=> {
						return (
							<div class="btn-group btn-block">
								<button type="button" class="btn btn-warning btn-sm" on-click={()=> this.$router.push(`/doctors/specialties/${row['id']}/edit`)}>
	...
	methods: {
		...
		async getDataFromApi() {
			...
			try {
				const { data } = await this.$axios.get(`doctor-specialties`, {
					params: {
						page: this.pageIndex,
		...
		deleteRow(id){
			...
			this.$swal.fire({
				title: 'Sure to delete specialty?',
				...
				if (result && !result.dismiss) {
					...
					try {
						...
						await this.$axios.delete(`doctor-specialties/${id}`);
				...
				if (result && result.dismiss) {
					this.$swal.fire('SPecialty are not deleted', '', 'info');
				}
			})
