<template>
	<div class="content-fluid">
		<portal to="header-title">
			<span>Area</span>
			<nuxt-link tag="a" to="/areas/create" class="btn btn-primary" style="margin-left:1%;">
	...
	data() {
		return {
			...
			columns: [
				...
				{ field: "area_name", key: "b", title: "Area", align: "left", sortBy: "" },
				{ 
				...
				{ 
					field: "region_id", key: "c", title: "Region", align: "left", sortBy: "", 
					renderBodyCell: ({row, column, rowIndex}, h)=> {
						const text = {
							region: `${row['region'].region_name}`,
							country: `${row['region']['country'].country_name} (${row['region']['country'].code})`
						}
						return (<div>
							<div>{text.region}</div>
							<div class="text-muted">{text.country}</div>
						</div>);
					},
				},
				{
					...
					renderBodyCell: ({row, column, rowIndex}, h)=> {
						return (
							<div class="btn-group btn-block">
								<button type="button" class="btn btn-warning btn-sm" on-click={()=> this.$router.push(`areas/${row['id']}/edit`)}>
	...
	methods: {
		...
		deleteRow(id){
			...
			this.$swal.fire({
				...
				if (result && result.dismiss) {
					this.$swal.fire('Area are not deleted', '', 'info');
				}
			})
