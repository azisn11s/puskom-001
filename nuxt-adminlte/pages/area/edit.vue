<template>
	<div class="content-fluid">
		<portal to="header-title">
			<span>Edit Area - #{{ form.region_name || "-" }}</span>
		</portal>
							...
							<div class="form-group row">
								...
								<div class="col-sm-8">
									<v-select
										...
										:class="{
											'is-invalid': formErrors.has('region_id'),
										}"
							...
							<div class="form-group row">
								...
								<div class="col-sm-8">
									<input
										...
										:class="{
											'is-invalid': form.errors.has('area_name'),
										}"
									/>
									<has-error
										:form="form"
...
export default {
	...
	async asyncData({ params, $axios }) {
		const area = await $axios.get(`areas/${params.id}`).then((resp) => {
			return resp.data.data;
		});

		const selectedRegion = await $axios
			.get(`regions/${area.region_id}`)
			.then((resp) => {
				return {
		...
		const selectedEmployee = (!area.employee_id) ? null : await $axios
			.get(`employees/${area.employee_id}`)
			.then((resp) => {
				return {
					code: resp.data.data.id,
	...
	methods: {
		async edit() {
			...
			this.isLoading = true;
			try {
				await this.$axios.put(`areas/${this.$route.params.id}`, this.form);
